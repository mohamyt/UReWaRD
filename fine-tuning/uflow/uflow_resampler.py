...
def resampler_with_unstacked_warp(data,
  ...
  with tf.name_scope(name):
    ...
    warp_batch_dim = warp_x.shape[0]
    data_batch_dim = data.shape[0]
    if (warp_batch_dim is not None and data_batch_dim is not None and
        warp_batch_dim != data_batch_dim):
      raise ValueError(
          '\'warp_x\' and \'data\' must have compatible first '
          'dimension (batch size), but their shapes are %s and %s ' %
          (str(warp_batch_dim), str(data_batch_dim)))
