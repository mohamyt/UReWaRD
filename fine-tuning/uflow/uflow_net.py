...
class UFlow(object):
  ...
  def update_checkpoint_dir(self, checkpoint_dir):
    ...
    if checkpoint_dir:
      ...
      tf.io.gfile.makedirs(milestone_dir)
      self._milestone_manager = tf.train.CheckpointManager(
          self._checkpoint, directory=milestone_dir, max_to_keep=10)
    else:
  ...
  def save(self):
    ...
    if self._milestone_manager is not None:
      step = tf.compat.v1.train.get_or_create_global_step().numpy()
      if step % 50000 == 0:
        self._milestone_manager.save()
        print(f'Milestone checkpoint saved at step {step}')
