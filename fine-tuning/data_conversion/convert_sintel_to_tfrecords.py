...
def write_records(data_list, output_folder):
  ...
  # Give this process its own scratch folder, so parallel conversions do not
  # delete each other's temporary image/flow files.
  tmpdir = tempfile.mkdtemp(prefix='uflow_sintel_')
  try:
    filenames = conversion_utils.generate_sharded_filenames(
        os.path.join(output_folder, 'sintel@{}'.format(FLAGS.num_shards)))
    record_writer = tf.io.TFRecordWriter(filenames[FLAGS.shard])
    with record_writer:
      total = len(data_list)
