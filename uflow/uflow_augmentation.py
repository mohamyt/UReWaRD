...
def _tfa_rotate(image, angle, interpolation='BILINEAR'):
  """Drop-in replacement for tfa_image.rotate using tf.raw_ops.
  Expects rank-4 [B, H, W, C] input (as passed by rotate())."""
  h = tf.shape(image)[1]
  w = tf.shape(image)[2]
  ...
  b0 =  sin_a
  b1 =  cos_a
  b2 = (1 - cos_a) * cy - sin_a * cx
  transform = tf.stack([a0, a1, a2, b0, b1, b2, tf.zeros_like(a0), tf.zeros_like(a0)])
  transform = tf.reshape(transform, [1, 8])
  transform = tf.tile(transform, [tf.shape(image)[0], 1])  # [B, 8]
  ...
  rotated = tf.raw_ops.ImageProjectiveTransformV3(
      ...
      fill_mode='CONSTANT',
      fill_value=0.0)
  return tf.cast(rotated, image.dtype)

from uflow import uflow_utils


def _apply_gaussian_blur(image, sigma):
  """Gaussian blur via depthwise conv with an 11x11 kernel of variable sigma."""
  k = 5
  x = tf.cast(tf.range(-k, k + 1), tf.float32)
  gauss_1d = tf.exp(-0.5 * tf.square(x / sigma))
  gauss_1d = gauss_1d / tf.reduce_sum(gauss_1d)
  gauss_2d = gauss_1d[:, None] * gauss_1d[None, :]        # [11, 11]
  kernel = tf.stack([gauss_2d] * 3, axis=2)[:, :, :, None]  # [11, 11, 3, 1]
  return tf.nn.depthwise_conv2d(
      image[None], kernel, strides=[1, 1, 1, 1], padding='SAME')[0]


@gin.configurable
def rwave_domain_augmentation(images,
                               apply_blur=False,
                               blur_sigma_min=0.5,
                               blur_sigma_max=3.0):
  """Gaussian blur for binary retinal wave images, applied before geometric aug.

  Converts hard binary edges into soft gradients closer to natural image
  statistics.  Defaults to off; enable via gin bindings in the slurm script.
  Noise and intensity scale are handled by photometric_augmentation.
  """
  def augment_single(img):
    if apply_blur:
      sigma = tf.random.uniform([], minval=blur_sigma_min, maxval=blur_sigma_max)
      img = _apply_gaussian_blur(img, sigma)
    return tf.clip_by_value(img, 0.0, 1.0)

  image_1, image_2 = tf.unstack(images, num=2)
  return tf.stack([augment_single(image_1), augment_single(image_2)])
...
def photometric_augmentation(images,
                             ...
                             min_bound_intensity_scale=0.5,
                             max_bound_intensity_scale=1.0,
  ...
  if augment_intensity_scale:
    scale = tf.random.uniform([],
                              minval=min_bound_intensity_scale,
                              maxval=max_bound_intensity_scale,
                              dtype=tf.float32)
    images = images * scale
