...
flags.DEFINE_float(
    'data_fraction', 1.0,
    'Fraction of training data to use (0 < x <= 1.0). '
    'For KITTI (multi-shard) truncates the shard file list. '
    'For Sintel and Chairs (single-shard) limits examples via dataset.take().')

flags.DEFINE_integer(
    'seed', 41,
    'Random seed for data shuffling (dataset.shuffle), TF global ops, and '
    'NumPy. Default 41 matches the original hardcoded value. Change to '
    'produce statistically independent runs for robustness checks.')
