...
"""Utilities for KITTI dataset loading and evaluation."""
...
def parse_eval_data(proto):
  ...
  return images, flow_uv_occ, flow_uv_noc, flow_valid_occ, flow_valid_noc
...
def make_dataset(path,
  ...
  Args:
    ...
    mode: string, one of ['train', 'eval', 'test'] to switch between loading
      training data, evaluation data, and test data, which have different
      formats.
    seq_len: int, how many images from the sequence to return.
    ...
    height: int, height for reshaping the images (only if for_eval=False)
      ...
      function through uflow.inference.
...
def benchmark(inference_fn,
  ...
  Args:
    ...
    plot_dir: string, optional path to a directory in which plots are saved (if
      num_plots > 0).
    unused_num_plots: int, maximum number of qualitative results to plot for the
      evaluation (unused).
  ...
  for i, test_batch in enumerate(it):
    ...
    image_batch = test_batch

    start_in_ms = time.time() * 1000
    flow = inference_fn(
        image_batch[0], image_batch[1], input_height=height, input_width=width)
    ...
    flow_uint16 = tf.concat(
        [flow_uint16,
         tf.ones_like(flow_uint16[Ellipsis, :1], dtype=tf.uint16)],
        axis=-1)
