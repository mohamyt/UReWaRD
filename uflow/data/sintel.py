...
def make_dataset(path,
  ...
  if 'train' in mode:
    ...
    if data_fraction < 1.0:
      files = files[:max(1, int(len(files) * data_fraction))]
  num_files = len(files)
  ...
  for data_type in ['clean', 'final']:
    ...
    for image_folder in image_folders:
      ...
      for images in image_pairs:
        ...
        if tf.io.gfile.exists(flow_output_path):
          continue  # Keep finished flow files when a benchmark run is resumed.
        image1_data = imageio.imread(img1_path)  # Read the first PNG frame.
        image2_data = imageio.imread(img2_path)  # Read the second PNG frame.
